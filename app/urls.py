...
urlpatterns = [
  ...
  path('quiz/GoodQuiz/<int:quiz_pk>/<slug:slug>', good_quiz_view, name='good_quiz'), # gqの処理のみ
  path('quiz/BetaMon/<int:quiz_pk>/<slug:slug>', betamon_view, name='betamon'), # betamonの処理のみ
  path('quiz/user/draft/list', draft_list_view, name='draft_list'),
  path('quiz/<slug:slug>/quiz/list', user_quiz_list_view, name='user_quiz_list'),
